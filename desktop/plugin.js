...
${Ud(this.phase.value)}</pre>
  ...
  `}var G=100,U=40,A=2;function it(e,t,r){let a=r-t;if(a<=0)return U/2;let s=(e-t)/a;return U-A-s*(U-A*2)}function Ne(e,t){return t<=1?G/2:A+e/(t-1)*(G-A*2)}function Ht(e,t,r){return e.map((a,s)=>`${Ne(s,e.length)},${it(a,t,r)}`).join(" ")}var sl=new Intl.NumberFormat("en-US",{notation:"compact",maximumFractionDigits:1});function ge(e){return Number.isFinite(e)?sl.format(e):""}function Ha(e){if(e.length<2)return"flat";let t=e[0],r=e[e.length-1];return r>t?"up":r<t?"down":"flat"}function ol(e){return _`<polyline
    class="dashboard-chart__line"
    fill="none"
    points=${Ht(e.values,e.min,e.max)}
  />`}function nl(e){let t=Ht(e.values,e.min,e.max),r=Ne(0,e.values.length),a=Ne(e.values.length-1,e.values.length),s=U-A;return _`<g>
    <polygon class="dashboard-chart__area" points=${`${r},${s} ${t} ${a},${s}`} />
    <polyline class="dashboard-chart__line" fill="none" points=${t} />
  </g>`}function il(e){let t=e.values.length,r=(G-A*2)/t,a=r>3?Math.min(1,r*.2):0,s=Math.max(r-a,.5),o=U-A;return _`<g class="dashboard-chart__bars">
    ${e.values.map((n,i)=>{let l=it(n,e.min,e.max);return _`<rect x=${A+i*r+a/2} y=${l} width=${s} height=${Math.max(o-l,0)} />`})}
  </g>`}function dl(e,t){let r=e.values.length?e.values[e.values.length-1]:0,a=T(t.min)??Math.min(e.min,0),s=(T(t.max)??Math.max(e.max,r))-a,o=s>0?Math.min(Math.max((r-a)/s,0),1):0,n=G/2,i=U-A,l=Math.min(G/2,U)-A,c=v=>{let w=Math.PI-v*Math.PI;return{x:n+l*Math.cos(w),y:i-l*Math.sin(w)}},b=c(0),h=c(1),y=c(o);return _`<g class="dashboard-chart__gauge">
  ...
  </g>`}function ll(e){let t=e.values.length,r=Ha(e.values);return t<2?_`<g class="dashboard-chart__spark dashboard-chart__spark--${r}">
  ...
  </g>`}function ja(e){return e==="line"||e==="area"||e==="bar"}function cl(){return _`<g class="dashboard-chart__grid">
    ${[A,U/2,U-A].map(e=>_`<line x1=${A} y1=${e} x2=${G-A} y2=${e} />`)}
  </g>`}function ul(e){let t=e.values.length;if(e.type==="bar"){let r=(G-A*2)/t;return _`<g class="dashboard-chart__tips">
  ...
  </g>`}function bl(e,t){switch(e.type){case"bar":return il(e);case"area":return nl(e);case"gauge":return dl(e,t);case"sparkline":return ll(e);default:return ol(e)}}function hl(e,t){let r=bl(e,t);return!e.detail||e.type==="sparkline"?r:_`<g>
    ${ja(e.type)?cl():p}
    ${r}
    ${ul(e)}
  </g>`}function pl(e,t){let r=ri(e,t);if(r.values.length===0)return u`<div class="dashboard-widget__placeholder">
    ...
    <div class="dashboard-chart dashboard-chart--${r.type}${i}">
      ...
      ${o?u`<span class="dashboard-chart__axis dashboard-chart__axis--max"
              ...
              >`:p}
      ${n?u`<span
              class="dashboard-chart__spark-value dashboard-chart__spark-value--${Ha(r.values)}"
              >${ge(r.values[r.values.length-1]??0)}</span
            >`:p}
    </div>
  `}var $t=class extends Da{constructor(e){if(super(e),this.it=p,e.type!==La.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(e){if(e===p||e==null)return this._t=void 0,this.it=e;if(e===ae)return e;if(typeof e!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(e===this.it)return this._t;this.it=e;let t=[e];return t.raw=t,this._t={_$litType$:this.constructor.resultType,strings:t,values:[]}}};$t.directiveName="unsafeHTML",$t.resultType=1;var qa=zt($t);function ye(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function gl(e){return/^https?:\/\//i.test(e.trim())}function Pe(e){let t=e;return t=t.replace(/`([^`]+)`/g,(r,a)=>`<code>${a}</code>`),t=t.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g,(r,a,s)=>gl(s)?`<a href="${s}" rel="noopener noreferrer">${a}</a>`:r),t=t.replace(/\*\*([^*]+)\*\*/g,(r,a)=>`<strong>${a}</strong>`),t=t.replace(/(^|[^*])\*([^*]+)\*/g,(r,a,s)=>`${a}<em>${s}</em>`),t=t.replace(/(^|[^_])_([^_]+)_/g,(r,a,s)=>`${a}<em>${s}</em>`),t}var fl=/^ {0,3}(#{1,6})(?:[ \t]+(.*))?$/;function ml(e){let t=e.split(`
`);return t.every(r=>r.startsWith(">"))?`<blockquote>${t.map(r=>Pe(ye(r.replace(/^>\s?/,"")))).join("<br>")}</blockquote>`:`<p>${t.map(r=>Pe(ye(r))).join("<br>")}</p>`}function yl(e){let t=/^\[([ xX])\]\s+(.*)$/.exec(e);if(!t)return`<li>${Pe(ye(e))}</li>`;let r=t[1]!==" ";return`<li class="dashboard-markdown__task-item">${`<span class="dashboard-markdown__task" role="img" aria-label="${r?"checked":"unchecked"}">${r?"\u2611":"\u2610"}</span>`} ${Pe(ye(t[2]))}</li>`}function Lr(e,t){let r=e.split(`
`).map(s=>s.replace(t?/^\s*\d+\.\s+/:/^\s*[-*]\s+/,"")).map(yl).join("");if(!t)return`<ul>${r}</ul>`;let a=Number.parseInt(/^\s*(\d+)\./.exec(e)?.[1]??"1",10);return Number.isSafeInteger(a)&&a!==1?`<ol start="${a}">${r}</ol>`:`<ol>${r}</ol>`}function Dr(e){return e.split(`
`).every(t=>/^\s*[-*]\s+/.test(t))}function Ur(e){return e.split(`
`).every(t=>/^\s*\d+\.\s+/.test(t))}function vl(e){return`<pre><code>${ye(e.join(`
`))}</code></pre>`}function Fa(e){let t=e.replace(/\r\n?/g,`
`).split(`
`),r=[],a=[],s="",o=()=>{if(a.length===0)return;let n=a.join(`
`);Dr(n)?r.push(Lr(n,!1)):Ur(n)?r.push(Lr(n,!0)):r.push(ml(n)),a=[]};for(let n=0;n<t.length;n+=1){let i=t[n];if(i.startsWith("```")){o();let h=[];for(n+=1;n<t.length&&!t[n].startsWith("```");)h.push(t[n]),n+=1;r.push(vl(h));continue}if(i.trim()===""){o();continue}let l=fl.exec(i);if(l){o();let h=l[1].length;r.push(`<h${h}>${Pe(ye(l[2]??""))}</h${h}>`);continue}let c=Dr(i)?"ul":Ur(i)?"ol":"p",b=/^(?: {4}| {0,3}\t)/.test(i)||/^\s*(?:[-*]|\d+\.)\s*$/.test(i)||c==="ol"&&Number.parseInt(i.trim(),10)!==1;s==="p"&&a.length>0&&b&&(c="p"),c!==s&&o(),s=c,a.push(i)}return o(),r.join(`
`)}function Va(e){return e.status==="ok"?"ok":e.status==="error"?"error":"pending"}function wl(e){return{turn:{turnId:e,items:[],status:"streaming"},textById:new Map,callById:new Map}}function _l(e){return e.items[e.items.length-1]}function _e(e){for(let t of e.items)t.kind==="error"&&t.retryable&&!t.superseded&&(t.superseded=!0)}function F(e,t,r){let a=e.get(r);return a||(a=wl(r),e.set(r,a),t.push(r)),a}function zr(e,t){let r=_l(e);if(r&&r.kind==="tools"){r.calls.push(t);return}e.items.push({kind:"tools",calls:[t]})}function xl(e){let t=new Map,r=[];for(let a of e)switch(a.type){case"turn-start":F(t,r,a.turnId);break;case"text-start":{let{turn:s,textById:o}=F(t,r,a.turnId);if(_e(s),!o.has(a.id)){let n={kind:"text",id:a.id,text:"",closed:!1};o.set(a.id,n),s.items.push(n)}break}case"text-delta":{let{turn:s,textById:o}=F(t,r,a.turnId);_e(s);let n=o.get(a.id);n||(n={kind:"text",id:a.id,text:"",closed:!1},o.set(a.id,n),s.items.push(n)),n.text+=a.delta;break}case"text-end":{let s=t.get(a.turnId)?.textById.get(a.id);s&&(s.closed=!0);break}case"tool-call-start":{let{turn:s,callById:o}=F(t,r,a.turnId);if(_e(s),!o.has(a.callId)){let n={callId:a.callId,name:a.name,argsText:"",status:"building"};o.set(a.callId,n),zr(s,n)}break}case"tool-call-delta":{let s=t.get(a.turnId)?.callById.get(a.callId);s&&(s.argsText+=a.argsTextDelta);break}case"tool-call-ready":{let{turn:s,callById:o}=F(t,r,a.turnId);_e(s);let n=o.get(a.callId);n||(n={callId:a.callId,name:a.name,argsText:"",status:"building"},o.set(a.callId,n),zr(s,n)),n.name=a.name,n.args=a.args,n.status="ready";break}case"tool-result":{let s=t.get(a.turnId)?.callById.get(a.callId);s&&(s.ok=a.ok,s.status=a.ok?"ok":"error",a.result!==void 0&&(s.result=a.result),a.error!==void 0&&(s.error=a.error));break}case"usage":{let{turn:s}=F(t,r,a.turnId);s.usage={inputTokens:a.inputTokens,outputTokens:a.outputTokens};break}case"abort":{let{turn:s}=F(t,r,a.turnId);s.status="aborted";break}case"turn-end":{let{turn:s}=F(t,r,a.turnId);if(s.stopReason!==void 0)break;s.stopReason=a.stopReason,s.status=a.stopReason==="aborted"?"aborted":"complete";break}case"error":{let{turn:s}=F(t,r,a.turnId??r[r.length-1]??"");_e(s),s.items.push({kind:"error",code:a.code,message:a.message,retryable:a.retryable,superseded:!1});break}default:break}return r.map(a=>t.get(a).turn)}function Ga(e){return typeof e=="object"&&e!==null}function xe(e){return typeof e=="string"?e:""}function Wr(e,t){let r=Ga(t)?t:{};switch(e.startsWith("dashboard.")?e.slice(10):e){case"tab.create":{let a=xe(r.title)||xe(r.slug);return a?d("dashboard.widget.chat.tool.createdTab",{name:a}):e}case"widget.add":{let a=xe(r.id)||xe(r.widgetId);return a?d("dashboard.widget.chat.tool.addedWidget",{id:a}):e}case"workspace.get":return d("dashboard.widget.chat.tool.readBoard");default:return e}}function $l(e){return e.map(t=>{let r=Va(t);return r==="ok"?"\u2713":r==="error"?"\u2717":"\xB7"}).join("")}function kl(e){return e===1?d("dashboard.widget.chat.actionsOne"):d("dashboard.widget.chat.actionsMany",{count:String(e)})}function Hr(e){try{return JSON.stringify(e,null,2)}catch{return String(e)}}function Al(e,t){let r=(e.status==="building"||e.status==="ready")&&!e.ok;if(r&&!t)return u`<div class="dashboard-chat__tool-row dashboard-chat__tool-row--building">
  ...
  </div>`}function El(e,t){let r=e.calls.length;return u`<details class="dashboard-chat__tools" data-test-id="dashboard-chat-tools">
    <summary class="dashboard-chat__chip">
      <span aria-hidden="true">🔧</span>
      <span class="dashboard-chat__chip-count">${kl(r)}</span>
      <span class="dashboard-chat__chip-sep" aria-hidden="true">·</span>
      <span class="dashboard-chat__chip-marks">${$l(e.calls)}</span>
    </summary>
    <div class="dashboard-chat__tool-log">
      ${e.calls.map(a=>Al(a,t))}
    </div>
  </details>`}function Tl(e){let t=e.status==="aborted";return u`<div
    ...
    <ul class="dashboard-list dashboard-cron" data-test-id="dashboard-cron">
      ${r.jobs.map(a=>u`
          <li class="dashboard-list__row ${a.enabled?"":"dashboard-list__row--disabled"}">
            ...
            ${a.lastStatus?u`<span class="dashboard-badge ${Il(a.lastStatus)}"
                    >${a.lastStatus}</span
                  >`:p}
          </li>
        `)}
    </ul>
  `}function Ka(e){return e==="scripts"?"allow-scripts":""}function Nl(e,t,r){let a=Ta(S(e).url,{allowExternalEmbedUrls:r.embed.allowExternalEmbedUrls});return a.status==="missing"?u`<div class="dashboard-widget__placeholder">
    ...
    loading="lazy"
  ></iframe>`}function Pl(e,t){let r=Gn(e,t);return r.instances.length===0?u`<div class="dashboard-widget__placeholder">
  ...
  `}function Ol(e,t){let r=Rn(e,t);return r.trim()?u`<div class="dashboard-markdown markdown-body">
    ${qa(Fa(r))}
  </div>`:u`<div class="dashboard-widget__placeholder">
      ${d("dashboard.widget.markdownEmpty")}
    </div>`}function qr(e){let t=S(e);return typeof t.text=="string"?t.text:""}function Bl(e,t){return r=>{if(!(r instanceof HTMLTextAreaElement))return;let a=r;if(a.dataset.notesPersisted!=="1"&&a.dataset.notesDirty!=="1"&&(a.value=t),a.dataset.notesBound==="1")return;a.dataset.notesBound="1",e.get().then(o=>{typeof o.state=="string"&&a.dataset.notesDirty!=="1"&&(a.dataset.notesPersisted="1",a.value=o.state)}).catch(()=>{});let s;a.addEventListener("input",()=>{a.dataset.notesDirty="1";let o=a.value;s!==void 0&&clearTimeout(s),s=setTimeout(()=>{e.set(o).catch(()=>{})},500)})}}function Ll(e,t,r){let a=d("dashboard.widget.notes.placeholder");if(!r.state){let s=qr(e);return u`
...
${s}</textarea>
  ...
  `}var Dl=["desktop","tablet","mobile"];function Fr(e){return`dashboard-preview__frame-wrap dashboard-preview__frame-wrap--${e}`}function Ul(e,t,r){let a=Ta(S(e).url,{allowExternalEmbedUrls:r.embed.allowExternalEmbedUrls});if(a.status==="missing")return u`<div class="dashboard-widget__placeholder">
  ...
  </div>`}function zl(e,t,r){let a=Dn(e,t);if(a.rows.length===0)return u`<div class="dashboard-widget__placeholder">
  ...
  `}function Wl(e,t){let r=Sn(e,t);return u`
  ...
  `}function Hl(e){return e==null?"":typeof e=="string"?e:typeof e=="number"||typeof e=="boolean"?String(e):JSON.stringify(e)}function jl(e,t){let r=Pn(e,t);if(r.total===0||r.columns.length===0)return u`<div class="dashboard-widget__placeholder">
    ...
    <div class="dashboard-table">
      <table class="dashboard-table__grid">
        ...
        <tbody>
          ${r.rows.map(s=>u`
              <tr>
                ${r.columns.map(o=>u`<td>${Hl(s[o])}</td>`)}
              </tr>
            `)}
        </tbody>
      </table>
  ...
  `}function ql(e,t){let r=Un(e,t);return u`
  ...
  `}var Fl={"stat-card":(e,t)=>Wl(e,t),markdown:(e,t)=>Ol(e,t),table:(e,t)=>jl(e,t),"iframe-embed":Nl,preview:Ul,sessions:(e,t,r)=>zl(e,t,r),usage:(e,t)=>ql(e,t),cron:(e,t)=>Cl(e,t),instances:(e,t)=>Pl(e,t),activity:(e,t)=>Yd(e,t),chart:(e,t)=>pl(e,t),notes:Ll,"action-form":qd,"action-button":Wd,"agent-status":(e,t)=>Xd(e,t),approvals:al,chat:Ml};function Vl(e){let t=e.startsWith("builtin:")?e.slice(8):e;return Fl[t]}function Gl(e){return e.replace(/\s*\(custom\)\s*$/iu,"").trim()||e}function Kl(e,t){let r=K(e.createdBy);return r?t?u`<span
  ...
  >`:p}function Yl(e){return e.ephemeral?u`<span
  ...
  `}function Zl(e,t,r){if(t&&"error"in t)throw new Error(t.error);let a=t&&"value"in t?t.value:void 0,s=Vl(e.kind);return s?s(e,a,r):e.kind.startsWith("custom:")?u`<div class="dashboard-widget__placeholder">
      ${d("dashboard.widget.customPlaceholder")}
    </div>`:u`<div class="dashboard-widget__placeholder">
    ${d("dashboard.widget.unknownKind",{kind:e.kind})}
  </div>`}function Ql(e,t){if(t.status==="approved")return t.manifest?Od({widget:e,manifest:t.manifest,context:t.host}):u`<div
  ...
  </div>`}function Ya(e,t,r,a,s){try{return e.kind.startsWith("custom:")&&s?Ql(e,s):Zl(e,t,r)}catch(o){let n=o instanceof Error?o.message:String(o);return u`
  ...
  `}function Vr(e,t,r,a){let s=e.key==="ArrowLeft"?"left":e.key==="ArrowRight"?"right":e.key==="ArrowUp"?"up":e.key==="ArrowDown"?"down":null;s&&(e.preventDefault(),a.onKeyboardNudge(t,r,s))}var Gr=10;function Xa(e){let t=2166136261;for(let r=0;r<e.length;r++)t^=e.charCodeAt(r),t=Math.imul(t,16777619);return(t>>>0)%360}function Ja(e){return e.length<=Gr?e:`${e.slice(0,Gr-1)}\u2026`}function Za(e){let t=new Set;for(let r of e.tabs)for(let a of r.widgets){let s=a.createdBy;s&&K(s)&&t.add(s)}return[...t].sort()}function tc(e,t){let r=K(e);return r?{actor:e,agentId:r,short:Ja(r),hue:Xa(e),dimmed:t!==null&&e!==t}:null}var rc={embedSandboxMode:"strict",allowExternalEmbedUrls:!1};function ac(e){return e?{embedSandboxMode:e.sandboxMode,allowExternalEmbedUrls:e.allowExternalUrls}:rc}function sc(){return{open:!1,loading:!1,error:null,entries:[],snapshots:new Map,selectedVersion:null,confirmRestore:!1,restoring:!1}}var Qa="boardstate:gallery-url:v1";function oc(e){try{return e?.getItem(Qa)??""}catch{return""}}function nc(e,t){try{e?.setItem(Qa,t)}catch{}}function le(e){return e instanceof Error&&e.message.trim()?e.message.trim():"Widget gallery error."}var es="boardstate:onboarding-dismissed:v1";function ic(e){try{return e?.getItem(es)==="1"}catch{return!1}}function dc(e){try{e?.setItem(es,"1")}catch{}}var Ye=new WeakMap,Xe=new WeakMap;function At(e){let t=Xe.get(e);t&&(document.removeEventListener("pointerdown",t.onPointerDown,!0),document.removeEventListener("keydown",t.onKeyDown,!0),Xe.delete(e))}function lc(e,t,r){let a=t.openMenuWidgetId!==null;if(a===Xe.has(e))return;if(!a){At(e);return}let s=()=>{t.openMenuWidgetId!==null&&(t.openMenuWidgetId=null,At(e),r())},o=i=>{let l=i.target;l instanceof Element&&l.closest(".dashboard-widget__menu, .dashboard-widget__menu-toggle")||s()},n=i=>{i.key==="Escape"&&(i.preventDefault(),s())};document.addEventListener("pointerdown",o,!0),document.addEventListener("keydown",n,!0),Xe.set(e,{onPointerDown:o,onKeyDown:n})}function cc(e){At(e),uc(e)}function uc(e){let t=Ye.get(e);if(t){for(let r of t.streamSubs.values())r.unsubscribe();t.streamSubs.clear()}}function bc(e,t){let r=Ye.get(e);return r||(r={openMenuWidgetId:null,drag:null,bindingResults:new Map,bindingLoads:new Set,bindingVersion:-1,streamSubs:new Map,streamValues:new Map,manifestCache:new Map,manifestLoads:new Set,dataVersion:0,dialog:null,onboardingDismissed:ic(t),collapsedTabGroups:new Set,lastPresenceSlug:null,history:sc(),gallery:null,highlightedAgent:null},Ye.set(e,r)),r}function hc(e){let t=Ye.get(e);t&&(t.dataVersion+=1)}function ts(e){let t=e.bindings;return t?Object.values(t)[0]??null:null}function pc(e,t){return e.workspaceVersion*1000003+t.dataVersion}function gc(e,t,r,a,s){if(!t){for(let n of e.streamSubs.values())n.unsubscribe();e.streamSubs.clear();return}let o=new Map;for(let n of a.widgets){let i=ts(n);i?.source==="stream"&&i.event&&o.set(n.id,i)}for(let[n,i]of e.streamSubs){let l=o.get(n);(!l||i.workspaceVersion!==r.workspaceVersion||i.event!==l.event||i.pointer!==l.pointer)&&(i.unsubscribe(),e.streamSubs.delete(n),e.streamValues.delete(n))}for(let[n,i]of o){if(e.streamSubs.has(n))continue;let l=pd(t,i,c=>{e.streamValues.set(n,c),e.bindingResults.set(n,c),s?.()});e.streamSubs.set(n,{workspaceVersion:r.workspaceVersion,event:i.event,...i.pointer!==void 0?{pointer:i.pointer}:{},unsubscribe:l})}}async function fc(e,t,r){let a=t.bindings??{},s=[];for(let o of r.inputs??[]){let n=a[o];if(!n)return{error:`Computed input not found: ${o}`};let i=await Ut(e,n);if("error"in i)return{error:i.error};s.push(i.value)}return hd(r.op??"",s,r.arg)}function mc(e,t,r,a,s){let o=pc(r,e);e.bindingVersion!==o&&(e.bindingResults.clear(),e.bindingLoads.clear(),e.bindingVersion=o),gc(e,t,r,a,s);for(let n of a.widgets){let i=ts(n);if(!(!i||e.bindingResults.has(n.id)||e.bindingLoads.has(n.id))){if(i.source==="stream"){let l=e.streamValues.get(n.id);l&&e.bindingResults.set(n.id,l);continue}e.bindingLoads.add(n.id),(i.source==="computed"?fc(t,n,i):Ut(t,i)).then(l=>{e.bindingResults.set(n.id,l),e.bindingLoads.delete(n.id),s?.()})}}}function yc(e){return{width:(e instanceof HTMLElement?e.querySelector(".dashboard-grid"):null)?.clientWidth??0}}function vc(e){if(e.key!=="Escape")return;let t=e.currentTarget.closest("details");t?.open&&(e.preventDefault(),t.open=!1,t.querySelector("summary")?.focus())}function wc(e){let t=e.currentTarget;if(!t.open)return;let r=s=>{s.target instanceof Node&&t.contains(s.target)||(t.open=!1,document.removeEventListener("pointerdown",r,!0))},a=()=>{t.open||(document.removeEventListener("pointerdown",r,!0),t.removeEventListener("toggle",a))};document.addEventListener("pointerdown",r,!0),t.addEventListener("toggle",a)}function _c(e,t,r,a){if(t.onboardingDismissed||r.tabs.some(o=>o.widgets.length>0))return p;let s=()=>{t.onboardingDismissed=!0,dc(e.storage),a()};return u`
  ...
  `}function rs(e,t,r,a){t.activeSlug=va(r,a),e.onNavigate?.(a),e.onRequestUpdate?.()}function xc(){return u`<svg
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    stroke-width="2"
  ...
  </svg>`}function $c(e){if(e<=0)return p;let t=d("dashboard.tabs.presence",{count:String(e)});return u`
    <span
  ...
  `}function kc(e){return e.kind==="agent"?d("dashboard.tabs.groupAgent",{agent:e.agentId??"agent"}):e.kind==="system"?d("dashboard.tabs.groupSystem"):d("dashboard.tabs.groupUser")}function Ac(e,t,r,a){let s=()=>e.onRequestUpdate?.(),o=rt(a),n=Vo(o),i=Fo(a),l=n.length>1,c=b=>Ui(e.host,b).length;return u`
    <nav class="dashboard-tabs" role="tablist" aria-label=${d("dashboard.tabs.label")}>
      ${l?n.map(b=>{let h=r.collapsedTabGroups.has(b.key),y=()=>{h?r.collapsedTabGroups.delete(b.key):r.collapsedTabGroups.add(b.key),s()},v=kc(b);return u`
                <div
  ...
  `}function Ec(e,t,r,a){let s=t.basePath??"";for(let o of a.widgets){let n=Ot(o.kind);!n||ya(r,o.kind)!=="approved"||e.manifestCache.has(n)||e.manifestLoads.has(n)||(e.manifestLoads.add(n),vd(s,n).then(i=>{e.manifestLoads.delete(n),i&&(e.manifestCache.set(n,i),t.onRequestUpdate?.())}))}}function Tc(e){let t=e.transport,r=e.sessionKey??"main";return({widgetKey:a,text:s})=>Ia({widgetKey:a,text:s,confirmPrompt:async o=>e.confirm?await e.confirm(o):typeof window<"u"?window.confirm(o):!1,sendPrompt:async o=>{if(!t)throw new Error("Not connected.");await t.request("chat.send",{sessionKey:r,message:o,deliver:!1})}})}function as(e,t,r,a){let s=e.transport,o={embed:ac(e.embed),dispatchPrompt:Tc(e),onActionError:n=>{t.actionError=n,e.onRequestUpdate?.()},approvals:_i(r,(n,i)=>void Fe(t,s,{name:n,decision:i}),(n,i,l)=>void od(t,s,{name:n,decision:i,...l?.tools!==void 0?{tools:l.tools}:{},...l?.autoConfirm!==void 0?{autoConfirm:l.autoConfirm}:{},...l?.expiresAt!==void 0?{expiresAt:l.expiresAt}:{}})),registryPending:Rc(r)};return s&&(o.state=Ic(s,a.id),o.chat=Mc(s,e.sessionKey??"main"),o.approveWidget=(n,i)=>void Fe(t,s,{name:n,decision:i}),o.actions=Sc(s,e.operator===!0)),o}function Sc(e,t){let r={invoke:async a=>{let s=await e.request("dashboard.action.invoke",a);return k(s)&&s.pending===!0?{kind:"pending",id:typeof s.id=="string"?s.id:"",expiresAt:typeof s.expiresAt=="string"?s.expiresAt:""}:{kind:"result",result:s}},subscribe:a=>e.addEventListener("dashboard.action.changed",s=>{k(s)&&typeof s.id=="string"&&a({id:s.id,status:s.status,connector:typeof s.connector=="string"?s.connector:"",tool:typeof s.tool=="string"?s.tool:""})})};return t&&(r.confirm=async a=>{let s=await e.request("dashboard.action.confirm",{id:a});return{result:k(s)&&"result"in s?s.result:s}},r.deny=async a=>{await e.request("dashboard.action.deny",{id:a})}),r}function Rc(e){return Object.entries(e.widgetsRegistry).filter(([,t])=>t.status==="pending").map(([t])=>t)}function Mc(e,t){let r=a=>a.sessionKey===t;return{send:async a=>({turnId:(await e.request("chat.send",{sessionKey:t,message:a})).turnId}),abort:async a=>{await e.request("chat.abort",{sessionKey:t,turnId:a})},history:async()=>((await e.request("chat.history.get",{sessionKey:t})).events??[]).filter(r),subscribe:a=>e.addEventListener(Eo,s=>{let o=s;o&&r(o)&&a(o)})}}function Ic(e,t){return{get:()=>e.request("dashboard.widget.state.get",{widgetId:t}),set:r=>e.request("dashboard.widget.state.set",{widgetId:t,state:r})}}function ss(e,t,r,a,s,o){let n=Ot(s.kind);return n?{status:ya(a,s.kind),manifest:r.manifestCache.get(n)??null,host:{transport:e.transport,basePath:e.basePath??"",sessionKey:e.sessionKey??"main",tabSlug:o,...e.confirm?{confirmPrompt:e.confirm}:{}},onApprove:()=>void Fe(t,e.transport,{name:n,decision:"approved"}),onReject:()=>void Fe(t,e.transport,{name:n,decision:"rejected"})}:null}function Cc(e){return[...e.history.snapshots.entries()].map(([t,r])=>({version:t,workspace:r}))}function Nc(e,t,r){let a=r.createdBy;if(!a)return;let s=K(a),o=No(r.id,Cc(t));return{actor:a,agentId:s,...o!==void 0?{firstSeenVersion:o}:{},...s?{logbookHref:e.logbookHref??null}:{}}}async function Pc(e,t){let r=()=>e.onRequestUpdate?.(),a=t.history;a.loading=!0,a.error=null,r();try{let s=await _d(e.transport);a.entries=s,s.length>0&&a.selectedVersion===null&&(a.selectedVersion=s[0].version),a.error=null}catch(s){a.error=s instanceof Error?s.message:String(s)}finally{a.loading=!1,r()}a.selectedVersion!==null&&await os(e,t,a.selectedVersion)}async function os(e,t,r){let a=t.history;if(!a.snapshots.has(r))try{let s=await xd(e.transport,r);s&&(a.snapshots.set(r,s),e.onRequestUpdate?.())}catch(s){a.error=s instanceof Error?s.message:String(s),e.onRequestUpdate?.()}}function Oc(e,t){t.history.open=!0,t.history.confirmRestore=!1,Pc(e,t),e.onRequestUpdate?.()}function Et(e,t){t.history.open=!1,t.history.confirmRestore=!1,e.onRequestUpdate?.()}function Bc(e,t,r){t.history.selectedVersion=r,os(e,t,r),e.onRequestUpdate?.()}function Lc(e,t){let r=t.manifest,a=r.preferredSize&&typeof r.preferredSize=="object"?r.preferredSize:{},s=Math.min(12,Math.max(1,Number(a.w)||6)),o=Math.max(1,Number(a.h)||4);return{x:0,y:(e?.widgets??[]).reduce((n,i)=>{let l=i.grid.y+i.grid.h;return l>n?l:n},0),w:s,h:o}}function Dc(e,t,r,a,s){if(mc(r,e.transport,a,s,e.onRequestUpdate??null),Ec(r,e,a,s),s.widgets.length===0)return u`
    ...
    `;if(s.layout==="full")return Uc(e,t,r,a,s);let o=ns(e,t,r,s),n=_a(s.widgets),i=n*56+Math.max(0,n-1)*12,l=Za(a).length>=2;return u`
    <div class="dashboard-grid" style="min-height: ${i}px" data-test-id="dashboard-grid">
      ${s.widgets.map(c=>{let b=ss(e,t,r,a,c,s.slug),h=Nc(e,r,c),y=r.drag,v=y?.widgetId===c.id,w=v&&y.mode==="move"?`translate(${y.pointerDx}px, ${y.pointerDy}px)`:void 0,f=l&&c.createdBy?tc(c.createdBy,r.highlightedAgent):null;return ec({widget:c,binding:r.bindingResults.get(c.id)??null,...h?{blame:h}:{},menuOpen:r.openMenuWidgetId===c.id,pending:t.pendingWidgetIds.has(c.id),dragging:v,...w?{dragTransform:w}:{},builtinContext:as(e,t,a,c),callbacks:o,...b?{custom:b}:{},...f?{agentChip:f}:{}})})}
      ${zc(r,s)}
    </div>
  `}function Uc(e,t,r,a,s){let o=s.widgets[0],n=ns(e,t,r,s),i=ss(e,t,r,a,o,s.slug);return u`
  ...
  `}function zc(e,t){let r=e.drag;return r?u`
    <div
  ...
  `}function Wc(e,t,r){let a=r.dialog;if(!a)return p;let s=()=>e.onRequestUpdate?.(),o=()=>{r.dialog=null,s()};if(a.kind==="editTitle"){let c=d("dashboard.widget.editTitleTitle");return Je(c,o,u`
      ...
      `)}let n=d("dashboard.widget.moveToTabTitle"),i=t.workspace?tt(t.workspace).filter(c=>c.slug!==a.slug):[];return Je(n,o,u`
    ...
    `)}function Hc(e){Dd(e.strings);let t=Vi(e.host),r=bc(e.host,e.storage);t.requestUpdate=e.onRequestUpdate??null,lc(e.host,r,()=>e.onRequestUpdate?.());let a=e.connected;return Ki(e.host,t,a?e.transport:null),Yi(e.host,a?e.transport:null,()=>{hc(e.host),a&&t.activeSlug&&Mr(e.host,e.transport,t.activeSlug),e.onRequestUpdate?.()}),a&&!t.loaded&&!t.loading&&!t.error&&ne(t,e.transport,{requestedSlug:e.initialTab??null}),a&&t.activeSlug&&r.lastPresenceSlug!==t.activeSlug&&(r.lastPresenceSlug=t.activeSlug,Mr(e.host,e.transport,t.activeSlug)),u`
  ...
  `}function jc(e,t,r){if(t.error)return u`
    ...
    ${Yc(e,t,r,s)}
    ${_c(e,r,a,()=>e.onRequestUpdate?.())}
    ${Ac(e,t,r,a)}
    ${qc(e,r,a)}
    ${Dc(e,t,r,a,s)}
  `:u`<div class="card lazy-view-state" role="status">
      <div class="card-sub">${d("dashboard.empty.noVisibleTabs")}</div>
    </div>`}function qc(e,t,r){let a=Za(r);if(a.length<2)return t.highlightedAgent=null,p;let s=n=>{t.highlightedAgent=t.highlightedAgent===n?null:n,e.onRequestUpdate?.()},o=t.highlightedAgent;return u`
    <div
  ...
  `}function Fc(e,t){if(typeof document>"u"||typeof URL.createObjectURL!="function")return;let r=new Blob([t],{type:"application/json"}),a=URL.createObjectURL(r),s=document.createElement("a");s.href=a,s.download=e,document.body.append(s),s.click(),s.remove(),URL.revokeObjectURL(a)}function Vc(e,t){nd(e.transport).then(r=>Fc(r.filename,r.json)).catch(r=>{t.actionError=r instanceof Error?r.message:String(r),e.onRequestUpdate?.()})}function Gc(e,t,r){let a=r.currentTarget,s=a.files?.[0];a.value="",s&&s.text().then(o=>id(t,e.transport,o))}function Kc(e,t){t.gallery={indexUrl:oc(e.storage),mode:"widgets",entries:null,selected:null,recipes:null,selectedRecipe:null,busy:!1,error:null},e.onRequestUpdate?.()}function Yc(e,t,r,a){let s=a.layout==="full",o=()=>void ad(t,e.transport,{slug:a.slug,layout:s?"grid":"full"});return u`
  ...
  `}function Xc(e){let t=Date.parse(e);if(!Number.isFinite(t))return e;let r=Math.round((Date.now()-t)/1e3);if(r<60)return"just now";let a=Math.round(r/60);if(a<60)return`${a}m ago`;let s=Math.round(a/60);if(s<24)return`${s}h ago`;let o=Math.round(s/24);if(o<7)return`${o}d ago`;try{return new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric"}).format(new Date(t))}catch{return e}}function Jc(e,t,r){let a=r.history;if(!a.open)return p;let s=d("dashboard.history.title"),o=a.selectedVersion!==null?a.snapshots.get(a.selectedVersion):void 0,n=a.entries[0]?.version??null;return Je(s,()=>Et(e,r),u`
      <div class="dashboard-history" data-test-id="dashboard-history">
        ...
        <div class="dashboard-history__body">
          ...
          <div class="dashboard-history__detail">
            ${a.selectedVersion===null?u`<div class="card-sub">${d("dashboard.history.emptyDetail")}</div>`:tu(e,t,r,a.selectedVersion,o)}
          </div>
        </div>
        <div class="bs-dialog__actions">
    ...
    `)}function Zc(e){let t=[];return e.added>0&&t.push(d("dashboard.history.summary.added",{count:String(e.added)})),e.removed>0&&t.push(d("dashboard.history.summary.removed",{count:String(e.removed)})),e.moved>0&&t.push(d("dashboard.history.summary.moved",{count:String(e.moved)})),e.retitled>0&&t.push(d("dashboard.history.summary.retitled",{count:String(e.retitled)})),e.tabsChanged>0&&t.push(d("dashboard.history.summary.tabs",{count:String(e.tabsChanged)})),t.length>0?t.join(" \xB7 "):d("dashboard.history.summary.minor")}function Qc(e){return e?u`<span class="dashboard-history__change">
    <span class="dashboard-history__change-label">${Zc(e)}</span>
  </span>`:p}function eu(e,t,r){let a=t.history;return a.loading&&a.entries.length===0?u`<div class="dashboard-history__list">
  ...
  `}function tu(e,t,r,a,s){let o=r.history,n=t.workspace,i=a===(o.entries[0]?.version??null);return s?u`
  ...
  />`,default:_`<rect x="4" y="5" width="16" height="14" rx="2" opacity="0.6" />`},ru={activity:"list","agent-status":"list",approvals:"list",sessions:"list",instances:"list",cron:"list",chat:"list",usage:"gauge","action-button":"button","action-form":"button","iframe-embed":"frame",preview:"frame"};function au(e){let t=e.startsWith("custom:")?"custom":e.replace(/^builtin:/,"");return u`<svg
    ...
    aria-hidden="true"
  >
    ${Yr[t]??Yr[ru[t]??"default"]}
  </svg>`}function su(e,t,r){let a=(t?e.tabs.find(o=>o.slug===t):void 0)??rt(e)[0]??e.tabs[0];if(!a||a.widgets.length===0)return u`<div class="dashboard-history__preview dashboard-history__preview--empty">
  ...
  `}function ou(e,t){let r=Mo(e,t);return r.length===0?u`<div class="card-sub" data-test-id="dashboard-history-diff-empty">
  ...
  `}function nu(e,t,r){let a=r.gallery;if(!a)return p;let s=()=>e.onRequestUpdate?.(),o=()=>{r.gallery=null,s()},n=f=>{a.indexUrl=f.currentTarget.value},i=f=>{a.mode=f,a.selected=null,a.selectedRecipe=null,a.error=null,s()},l=async()=>{let f=a.indexUrl.trim();if(f){a.busy=!0,a.error=null,a.selected=null,a.selectedRecipe=null,s();try{let[x,I]=await Promise.all([$d(f),Ad(f)]);a.entries=x,a.recipes=I,nc(e.storage,f)}catch(x){a.error=le(x)}finally{a.busy=!1,s()}}},c=async f=>{a.busy=!0,a.error=null,s();try{a.selected=await kd(f.manifestUrl)}catch(x){a.error=le(x)}finally{a.busy=!1,s()}},b=async f=>{a.busy=!0,a.error=null,s();try{a.selectedRecipe=await Ed(f.manifestUrl)}catch(x){a.error=le(x)}finally{a.busy=!1,s()}},h=async()=>{let f=a.selectedRecipe;if(f){a.busy=!0,a.error=null,s();try{if(!await dd(t,e.transport,f)){a.error=t.actionError??le(new Error("Install failed.")),a.busy=!1,s();return}let x=f.doc.tabs[0]?.slug;x&&(t.activeSlug=x,e.onNavigate?.(x)),r.gallery=null,s()}catch(x){a.error=le(x),a.busy=!1,s()}}},y=async()=>{let f=a.selected;if(f){a.busy=!0,a.error=null,s();try{await Td(e.transport,f);let x=t.workspace?Bt(t.workspace,t.activeSlug):void 0;e.transport&&x&&await e.transport.request("dashboard.widget.add",{tab:x.slug,widget:{kind:`custom:${f.name}`,title:f.title,grid:Lc(x,f)}}),await ne(t,e.transport,{silent:!0}),r.gallery=null,s()}catch(x){a.error=le(x),a.busy=!1,s()}}},v=()=>a.selected?du(a.selected,()=>{a.selected=null,s()},()=>void y(),a.busy):iu(a,f=>void c(f)),w=()=>a.selectedRecipe?cu(a.selectedRecipe,()=>{a.selectedRecipe=null,s()},()=>void h(),a.busy):lu(a,f=>void b(f));return Je(d("dashboard.gallery.title"),o,u`
    ...
    `)}function iu(e,t){return e.entries===null?p:e.entries.length===0?u`<div class="dashboard-gallery__empty">${d("dashboard.gallery.empty")}</div>`:u`
  ...
  `}function lu(e,t){return e.recipes===null?p:e.recipes.length===0?u`<div class="dashboard-gallery__empty">${d("dashboard.gallery.recipesEmpty")}</div>`:u`
  ...
  `}function cu(e,t,r,a){let s=Object.entries(e.grantsManifest);return u`
  ...
  `}var fe,jt=(fe=class extends he{constructor(...t){super(...t),this.transport=null,this.connected=!1,this.operator=!1}createRenderRoot(){return this}render(){return Hc({host:this,transport:this.transport,connected:this.connected,onRequestUpdate:()=>this.requestUpdate(),...this.strings?{strings:this.strings}:{},...this.onNavigate?{onNavigate:this.onNavigate}:{},...this.storage?{storage:this.storage}:{},...this.confirm?{confirm:this.confirm}:{},...this.embed?{embed:this.embed}:{},...this.basePath!==void 0?{basePath:this.basePath}:{},...this.initialTab!==void 0?{initialTab:this.initialTab}:{},...this.sessionKey!==void 0?{sessionKey:this.sessionKey}:{},...this.logbookHref!==void 0?{logbookHref:this.logbookHref}:{},operator:this.operator})}disconnectedCallback(){super.disconnectedCallback(),Xi(this),cc(this)}},fe.properties={transport:{attribute:!1},connected:{type:Boolean},strings:{attribute:!1},onNavigate:{attribute:!1},storage:{attribute:!1},confirm:{attribute:!1},embed:{attribute:!1},basePath:{type:String},initialTab:{type:String},sessionKey:{type:String},logbookHref:{type:String},operator:{type:Boolean}},fe);typeof customElements<"u"&&!customElements.get("boardstate-view")&&customElements.define("boardstate-view",jt);var me,qt=(me=class extends he{constructor(...t){super(...t),this.currentLabel="",this.agentLabel="",this.brandLabel="",this.overviewHref="",this.handleOverviewClick=r=>{r.defaultPrevented||r.button!==0||r.metaKey||r.ctrlKey||r.shiftKey||r.altKey||(r.preventDefault(),this.dispatchEvent(new CustomEvent("navigate",{detail:"overview",bubbles:!0,composed:!0})))}}createRenderRoot(){return this}render(){let t=this.currentLabel.trim(),r=this.agentLabel.trim(),a=this.brandLabel.trim();return u`
    ...
    `}},me.properties={currentLabel:{type:String},agentLabel:{type:String},brandLabel:{type:String},overviewHref:{type:String}},me);typeof customElements<"u"&&!customElements.get("boardstate-header")&&customElements.define("boardstate-header",qt);var is=`/*
...
`;var ls={"--bs-bg":["--ui-surface-background","--ui-bg-editor"],"--bs-bg-hover":["--ui-row-hover-background","--ui-control-hover-background"],"--bs-bg-muted":["--ui-bg-chrome","--ui-bg-tertiary"],"--bs-surface-muted":["--ui-bg-chrome","--ui-bg-tertiary"],"--bs-card":["--ui-bg-elevated","--ui-surface-background"],"--bs-card-highlight":["--ui-row-active-background","--ui-bg-elevated"],"--bs-border":["--ui-stroke-secondary"],"--bs-border-strong":["--ui-stroke-primary"],"--bs-input":["--ui-bg-card","--ui-control-active-background"],"--bs-text":["--ui-text-primary"],"--bs-text-strong":["--ui-text-primary"],"--bs-text-muted":["--ui-text-tertiary"],"--bs-text-dim":["--ui-text-quaternary","--ui-text-tertiary"],"--bs-muted":["--ui-text-tertiary"],"--bs-accent":["--ui-accent"],"--bs-accent-foreground":["--ui-bg-elevated","--ui-surface-background"],"--bs-ring":["--ui-accent"],"--bs-danger":["--ui-red"],"--bs-success":["--ui-green"],"--bs-warning":["--ui-yellow"]};function cs(e){let t=`var(${e[e.length-1]})`;for(let r=e.length-2;r>=0;r--)t=`var(${e[r]}, ${t})`;return t}function hu(e){let t=e.match(/[\d.]+/g);if(!t||t.length<3)return 0;let r=/^\s*color\(/i.test(e)?1:255,[a,s,o]=t.slice(0,3).map(n=>{let i=Number(n)/r;return i<=.03928?i/12.92:Math.pow((i+.055)/1.055,2.4)});return .2126*a+.7152*s+.0722*o}function us(e){return hu(e)<.4?"dark":"light"}function dt(e,t,r){return{schemaVersion:1,workspaceVersion:1,widgetsRegistry:{},prefs:{tabOrder:[e]},tabs:[{slug:e,title:t,icon:"layoutDashboard",hidden:!1,createdBy:"system",widgets:r}]}}var Be=(e,t,r,a,s,o,n)=>({id:e,kind:"builtin:markdown",title:t,grid:{x:r,y:a,w:s,h:o},collapsed:!1,hidden:!1,props:{markdown:n}}),z=(e,t,r,a,s,o,n,i={})=>({id:e,kind:t,title:r,grid:{x:a,y:s,w:o,h:n},collapsed:!1,hidden:!1,props:i}),pu=(e,t,r,a,s,o,n,i,l,c=null)=>({id:e,kind:"builtin:action-button",title:t,grid:{x:r,y:a,w:s,h:o},collapsed:!1,hidden:!1,props:{connector:n,tool:i,label:l,args:c}}),bs=[{id:"agent-hq",name:"Agent HQ",summary:"Live operations overview \u2014 usage, sessions, connected instances, and schedules.",doc:dt("board","Agent HQ",[Be("header","Overview",0,0,12,2,`# Agent HQ
Live operations for this Hermes agent.`),z("usage","builtin:usage","Usage",0,2,4,3),z("instances","builtin:instances","Instances",4,2,4,3),z("sessions","builtin:sessions","Sessions",8,2,4,5),z("cron","builtin:cron","Scheduled jobs",0,5,8,3)])},{id:"usage-cost",name:"Usage & Cost",summary:"Spend and token usage at a glance, with the underlying breakdown.",doc:dt("board","Usage & Cost",[Be("header","Overview",0,0,12,2,`# Usage & Cost
Today's spend and token consumption.`),z("cost","builtin:stat-card","Cost",0,2,3,2,{metric:"todayCost",format:"usd",label:"Cost (today)"}),z("tokens","builtin:stat-card","Tokens",3,2,3,2,{metric:"todayTokens",format:"int",label:"Tokens (today)"}),z("usage","builtin:usage","Usage detail",6,2,6,3),z("cron","builtin:cron","Scheduled jobs",0,5,12,3)].map(e=>e.id==="cost"||e.id==="tokens"?{...e,bindings:{value:{source:"rpc",method:"usage.status"}}}:e))},{id:"sessions-monitor",name:"Sessions Monitor",summary:"Watch active sessions and connected instances in real time.",doc:dt("board","Sessions Monitor",[Be("header","Overview",0,0,12,2,`# Sessions Monitor
Active sessions and connected instances.`),z("sessions","builtin:sessions","Sessions",0,2,7,5),z("instances","builtin:instances","Instances",7,2,5,3),z("usage","builtin:usage","Usage",7,5,5,2)])},{id:"office-ops",name:"Office Ops",summary:"Operate OfficeCLI \u2014 generate documents and workbooks through approved tools, artifacts on the board.",doc:dt("board","Office Ops",[Be("header","Overview",0,0,12,2,"# Office Ops\nDrive **OfficeCLI** (`officecli mcp`) through operator-approved tools. Author the `officecli` connector in `boardstate.connectors.json`, approve its tools in the approvals panel, then act below."),pu("generate-report","Quarterly report",0,2,4,3,"officecli","officecli","Generate quarterly report .docx",{command:"create quarterly-report.docx"}),z("approvals","builtin:approvals","Approvals",4,2,8,3),Be("setup","Setup",0,5,12,2,"### Setup\n1. Install OfficeCLI (`brew install officecli` or a GitHub release) so `officecli` is on PATH.\n2. Author `boardstate.connectors.json` in the state dir with the `officecli` stdio connector.\n3. Approve the tools you want in the approvals panel \u2014 nothing runs until you do.")])}];var gu=new Set(["dashboard.widget.approve","dashboard.capability.approve","dashboard.action.confirm","dashboard.action.deny"]);function hs(e,t){return{request(a,s,o){return gu.has(a)?t(a,s??{}):e.request(a,s,o)},addEventListener(a,s){return e.addEventListener(a,s)},close(){e.close()},get ready(){return e.ready},get closed(){return e.closed}}}import{jsx as ve,jsxs as Vt}from"react/jsx-runtime";function _u(){customElements.get("boardstate-view")||customElements.define("boardstate-view",jt),customElements.get("boardstate-header")||customElements.define("boardstate-header",qt)}function xu(){if(document.querySelector("style[data-boardstate]"))return null;let t=document.createElement("style");return t.setAttribute("data-boardstate",""),t.textContent=`${is}
${ds}`,document.head.appendChild(t),t}function gs(e){let t=getComputedStyle(document.body).backgroundColor||"rgb(0,0,0)";e.setAttribute("data-theme",us(t));for(let[r,a]of Object.entries(ls))e.style.setProperty(r,cs(a));e.style.setProperty("--bs-radius-lg","var(--radius-xl, 10px)"),e.style.setProperty("--bs-radius-md","0.375rem"),e.style.setProperty("--bs-radius-sm","0.25rem"),e.style.setProperty("--bs-shadow-md","0 1px 3px rgba(0,0,0,0.10)"),e.style.setProperty("--bs-font-sans",getComputedStyle(document.body).fontFamily)}function $u(e,t,r,a){let s=new Map,o=!1,n=!1,i=r(()=>{!o&&!n&&a("degraded","Board requests work, but live updates are unavailable on this connection.")},2500),l=t("/ws",c=>{if(typeof c!="object"||c===null)return;let b=c;if(b.event==="boardstate.desktop.connected"){n=!0,i(),a("live");for(let h of s.get("boardstate.changed")??[])h({});return}if(typeof b.event=="string")for(let h of s.get(b.event)??[])h(b.payload)});return{ready:Promise.resolve(),get closed(){return o},async request(c,b){if(o)throw new Error("Boardstate transport is closed");let h=await e("/rpc",{method:"POST",body:{method:c,params:b??{}}});if(h&&h.error)throw new Error(String(h.error));return h?.result},addEventListener(c,b){let h=s.get(c)??new Set;return h.add(b),s.set(c,h),()=>{h.delete(b),h.size||s.delete(c)}},close(){o||(o=!0,i(),l(),s.clear())}}}function ku({rest:e,socket:t,setTimer:r}){let a=ps(null),s=ps(void 0),[o,n]=Ft("connecting"),[i,l]=Ft(""),[c,b]=Ft(""),h=vu(async(v,w)=>{let f=s.current;if(f&&window.confirm(`Replace the current board with the "${v}" template?`)){b(v);try{await f.request("dashboard.workspace.replace",{doc:w,actor:"user"})}catch(x){fu.notify?.({kind:"error",message:`Template failed: ${x instanceof Error?x.message:String(x)}`})}finally{b("")}}},[]);return wu(()=>{let v=!1,w,f,x,I=!1,Le=(W,Z="")=>{W==="error"&&(I=!0),!(I&&W!=="error")&&(v||(n(W),l(Z)))};return(async()=>{let W=async(Z,lt)=>{let g=await e("/operator",{method:"POST",body:{method:Z,params:lt}});if(g?.error)throw new Error(String(g.error));return g?.result};w=hs($u(e,t,r,Le),W),s.current=w,f=document.createElement("boardstate-view"),f.transport=w,f.connected=!0,f.operator=!0;try{let Z=await e("/assets-base",{method:"GET"});f.basePath=Z?.absoluteBase??""}catch{f.basePath=""}v||(gs(f),x=new MutationObserver(()=>f&&gs(f)),x.observe(document.documentElement,{attributes:!0,attributeFilter:["class","style","data-theme"]}),x.observe(document.body,{attributes:!0,attributeFilter:["class","style"]}),f.style.display="block",f.style.height="100%",a.current?.appendChild(f),await w.request("dashboard.workspace.get",{}))})().catch(W=>{Le("error",W instanceof Error?W.message:String(W))}),()=>{v=!0,x?.disconnect(),s.current=void 0;try{w?.close()}catch{}f&&f.parentNode&&f.parentNode.removeChild(f)}},[e,r,t]),Vt("div",{style:{display:"flex",flexDirection:"column",height:"100%",gap:8,padding:12},children:[Vt("div",{style:{display:"flex",alignItems:"center",flexWrap:"wrap",gap:8,fontSize:12},children:[ve("span",{style:{width:8,height:8,borderRadius:"50%",background:o==="live"?"var(--ui-green, #6aa84f)":o==="error"?"var(--ui-red, #e06c75)":"var(--ui-yellow, #d0a94f)",display:"inline-block"}}),ve("span",{style:{opacity:.8},children:o==="live"?"Board connected":o==="degraded"?i:o==="error"?`Board unavailable${i?`: ${i}`:""}`:"Connecting to board\u2026"}),o==="live"||o==="degraded"?Vt("span",{style:{display:"flex",alignItems:"center",flexWrap:"wrap",gap:6,marginLeft:8},children:[ve("span",{style:{opacity:.7},children:"Templates:"}),bs.map(v=>ve("button",{type:"button",title:v.summary,disabled:c!=="",onClick:()=>h(v.name,v.doc),style:{cursor:c?"default":"pointer",padding:"3px 10px",borderRadius:6,border:"1px solid var(--ui-stroke-secondary, #2a2a33)",background:c===v.id?"var(--ui-row-active-background, #23232b)":"transparent",color:"inherit",opacity:c&&c!==v.name?.5:1},children:c===v.name?"Applying\u2026":v.name},v.id))]}):null]}),ve("div",{ref:a,style:{flex:1,minHeight:0}})]})}var nb={id:"boardstate",name:"Board",register(e){_u();let t=xu();t&&e.onDispose(()=>t.remove());let r=typeof e.setTimeout=="function"?e.setTimeout:(a,s)=>{let o=globalThis.setTimeout(a,s),n=()=>globalThis.clearTimeout(o);return e.onDispose(n),n};e.register({id:"board-route",area:mu,data:{path:"/board"},render:()=>ve(ku,{rest:e.rest,socket:e.socket,setTimer:r})}),e.register({id:"board-nav",area:yu,data:{path:"/board",label:"Board",codicon:"dashboard"}})}};export{nb as default};
