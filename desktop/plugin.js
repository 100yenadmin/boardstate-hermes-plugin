...
${Ld(this.phase.value)}</pre>
    ...
    `}},xt=new Map;function Ud(e,t,r){let a=xt.get(e.id);return a||(a=new Dd(e.id),xt.set(e.id,a)),a.setContext(r,e),u`<div class="dashboard-action-button-host" ${Ne(a.rootRef)}></div>`}function zd(e){let t=e.type==="select"?u`<select class="dashboard-action-form__control" name=${e.name}>
  ...
  `}var G=100,U=40,A=2;function it(e,t,r){let a=r-t;if(a<=0)return U/2;let s=(e-t)/a;return U-A-s*(U-A*2)}function Pe(e,t){return t<=1?G/2:A+e/(t-1)*(G-A*2)}function Ht(e,t,r){return e.map((a,s)=>`${Pe(s,e.length)},${it(a,t,r)}`).join(" ")}var rl=new Intl.NumberFormat("en-US",{notation:"compact",maximumFractionDigits:1});function ge(e){return Number.isFinite(e)?rl.format(e):""}function Ua(e){if(e.length<2)return"flat";let t=e[0],r=e[e.length-1];return r>t?"up":r<t?"down":"flat"}function al(e){return _`<polyline
    class="dashboard-chart__line"
    fill="none"
    points=${Ht(e.values,e.min,e.max)}
  />`}function sl(e){let t=Ht(e.values,e.min,e.max),r=Pe(0,e.values.length),a=Pe(e.values.length-1,e.values.length),s=U-A;return _`<g>
    ...
    <line class="dashboard-chart__gauge-needle" x1=${n} y1=${i} x2=${y.x} y2=${y.y} />
      ...
      <circle class="dashboard-chart__spark-dot" cx=${Pe(0,t)} cy=${it(e.values[0]??0,e.min,e.max)} r="1.5" />
    ...
    ${e.values.map((r,a)=>_`<circle class="dashboard-chart__tip" cx=${Pe(a,t)} cy=${it(r,e.min,e.max)} r="2.5"><title>${ge(r)}</title></circle>`)}
...
`)}function ja(e){return e.status==="ok"?"ok":e.status==="error"?"error":"pending"}function yl(e){return{turn:{turnId:e,items:[],status:"streaming"},textById:new Map,callById:new Map}}function vl(e){return e.items[e.items.length-1]}function _e(e){for(let t of e.items)t.kind==="error"&&t.retryable&&!t.superseded&&(t.superseded=!0)}function F(e,t,r){let a=e.get(r);return a||(a=yl(r),e.set(r,a),t.push(r)),a}function Dr(e,t){let r=vl(e);if(r&&r.kind==="tools"){r.calls.push(t);return}e.items.push({kind:"tools",calls:[t]})}function wl(e){let t=new Map,r=[];for(let a of e)switch(a.type){case"turn-start":F(t,r,a.turnId);break;case"text-start":{let{turn:s,textById:o}=F(t,r,a.turnId);if(_e(s),!o.has(a.id)){let n={kind:"text",id:a.id,text:"",closed:!1};o.set(a.id,n),s.items.push(n)}break}case"text-delta":{let{turn:s,textById:o}=F(t,r,a.turnId);_e(s);let n=o.get(a.id);n||(n={kind:"text",id:a.id,text:"",closed:!1},o.set(a.id,n),s.items.push(n)),n.text+=a.delta;break}case"text-end":{let s=t.get(a.turnId)?.textById.get(a.id);s&&(s.closed=!0);break}case"tool-call-start":{let{turn:s,callById:o}=F(t,r,a.turnId);if(_e(s),!o.has(a.callId)){let n={callId:a.callId,name:a.name,argsText:"",status:"building"};o.set(a.callId,n),Dr(s,n)}break}case"tool-call-delta":{let s=t.get(a.turnId)?.callById.get(a.callId);s&&(s.argsText+=a.argsTextDelta);break}case"tool-call-ready":{let{turn:s,callById:o}=F(t,r,a.turnId);_e(s);let n=o.get(a.callId);n||(n={callId:a.callId,name:a.name,argsText:"",status:"building"},o.set(a.callId,n),Dr(s,n)),n.name=a.name,n.args=a.args,n.status="ready";break}case"tool-result":{let s=t.get(a.turnId)?.callById.get(a.callId);s&&(s.ok=a.ok,s.status=a.ok?"ok":"error",a.result!==void 0&&(s.result=a.result),a.error!==void 0&&(s.error=a.error));break}case"usage":{let{turn:s}=F(t,r,a.turnId);s.usage={inputTokens:a.inputTokens,outputTokens:a.outputTokens};break}case"abort":{let{turn:s}=F(t,r,a.turnId);s.status="aborted";break}case"turn-end":{let{turn:s}=F(t,r,a.turnId);if(s.stopReason!==void 0)break;s.stopReason=a.stopReason,s.status=a.stopReason==="aborted"?"aborted":"complete";break}case"error":{let{turn:s}=F(t,r,a.turnId??r[r.length-1]??"");_e(s),s.items.push({kind:"error",code:a.code,message:a.message,retryable:a.retryable,superseded:!1});break}default:break}return r.map(a=>t.get(a).turn)}function qa(e){return typeof e=="object"&&e!==null}function xe(e){return typeof e=="string"?e:""}function Ur(e,t){let r=qa(t)?t:{};switch(e.startsWith("dashboard.")?e.slice(10):e){case"tab.create":{let a=xe(r.title)||xe(r.slug);return a?d("dashboard.widget.chat.tool.createdTab",{name:a}):e}case"widget.add":{let a=xe(r.id)||xe(r.widgetId);return a?d("dashboard.widget.chat.tool.addedWidget",{id:a}):e}case"workspace.get":return d("dashboard.widget.chat.tool.readBoard");default:return e}}function _l(e){return e.map(t=>{let r=ja(t);return r==="ok"?"\u2713":r==="error"?"\u2717":"\xB7"}).join("")}function xl(e){return e===1?d("dashboard.widget.chat.actionsOne"):d("dashboard.widget.chat.actionsMany",{count:String(e)})}function zr(e){try{return JSON.stringify(e,null,2)}catch{return String(e)}}function $l(e,t){let r=(e.status==="building"||e.status==="ready")&&!e.ok;if(r&&!t)return u`<div class="dashboard-chat__tool-row dashboard-chat__tool-row--building">
